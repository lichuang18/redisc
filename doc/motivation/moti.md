...
\subsection{Discard Command Creation and Aggregation}

This experiment first evaluates whether ReDisc improves the overall shape of discard commands created by F2FS. We compare ReDisc with stock F2FS under two representative filesystem workloads, \texttt{varmail} and \texttt{fileserver}, and two discard-granularity configurations, 16~KB and 512~KB. We focus on two metrics: the number of newly created discard command objects and the average number of blocks covered by each created command. The first metric reflects how often F2FS has to allocate a new discard command instead of merging a newly discovered discardable range into an existing pending command. The second metric reflects command aggregation efficiency, i.e., whether each created command covers a larger discardable range.

Figure~\ref{fig:discard_creation} shows that ReDisc substantially reduces the number of newly created discard commands under the \texttt{varmail} workload. Compared with stock F2FS, ReDisc reduces the command count by 77.5% under the 16~KB configuration and by 81.2% under the 512~KB configuration. This reduction should not be interpreted as a reduction in the invalid blocks generated by the workload. Instead, it indicates that ReDisc allows more discardable ranges to be absorbed into existing pending commands before new command objects have to be created. In stock F2FS, a short discard range may be issued before nearby follow-up ranges become visible, causing later ranges to allocate additional command objects. ReDisc preserves eligible pending commands for a bounded interval, giving nearby follow-up ranges a better chance to merge into existing commands. As a result, the same local discard demand can be represented by fewer command objects.

The improvement is more modest under the \texttt{fileserver} workload. ReDisc reduces the created command count by 11.7% under the 16~KB configuration and by 4.9% under the 512~KB configuration. This workload-dependent behavior is expected. The \texttt{fileserver} workload has stronger spatial locality and tends to generate larger or more naturally contiguous invalidated regions. Therefore, stock F2FS already has more opportunities to aggregate discard ranges, leaving less room for ReDisc to further reduce command creation. In contrast, \texttt{varmail} performs many small-file and metadata-intensive operations, which produce short, fragmented, and temporally clustered discardable ranges. These conditions expose more merge opportunities for ReDisc to preserve.

The average number of blocks covered by each created command further supports this interpretation. Under \texttt{varmail} with the 16~KB configuration, ReDisc increases the average command coverage from 12.11 blocks to 52.23 blocks, a 4.3$\times$ improvement. This result shows that ReDisc does not merely reduce the number of command objects; it also improves the shape of created discard commands by allowing each command to cover a larger range. The \texttt{fileserver} workload shows only limited improvement in this metric, which is consistent with its more regular baseline discard behavior.

Overall, this experiment validates the first observable effect of ReDisc: it reduces discard command fragmentation at the command-creation level. ReDisc does not need to reduce the workload's original invalidation demand to achieve this effect. Instead, by preserving merge opportunities in the pending discard structure, ReDisc allows later discardable ranges to merge into existing commands and thereby reduces the number of newly created command objects. This effect is especially pronounced for small-file-intensive workloads such as \texttt{varmail}, where discardable ranges appear as short and locally correlated fragments. The following experiments further decompose this behavior and examine how SWOD, WCE, and SFI contribute to the observed improvement.

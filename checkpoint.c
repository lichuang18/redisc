...
int f2fs_write_checkpoint(struct f2fs_sb_info *sbi, struct cp_control *cpc)
{
	...
	// 不是resize，加全局cp lock，  
	//也就是说普通ckpt，这里加锁，  resize可能在外层拿到过lock
	if (cpc->reason != CP_RESIZE)
		down_write(&sbi->cp_global_sem);

	// 如果文件系统不脏，而且某些触发原因下没必要写，就直接跳出
...
int f2fs_issue_checkpoint(struct f2fs_sb_info *sbi)
{
	...
	// 过了上面的判断，说明就是启用了 MERGE 且是 CP_SYNC
	// 接下来判断ckpt线程是否存在，如果不存在退化为同步写
	if (!cprc->f2fs_issue_ckpt)
		return __write_checkpoint_sync(sbi); //这里退化为同步写

	// 线程存在，异步处理
	init_ckpt_req(&req);

	llist_add(&req.llnode, &cprc->issue_list);//加入队列
